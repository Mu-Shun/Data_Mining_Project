...
RDD = sc.textFile(input_file).map(lambda line:tuple(line.split(','))).distinct().map(lambda x: (x[0],[x[1]]))
RDD = RDD.filter(lambda x: x[0] != "user_id").reduceByKey(lambda a, b: a+b).filter(lambda x: len(x[1]) > filter_threshold).map(lambda x: (x[0], set(x[1])))

num_of_partition = RDD.getNumPartitions()
# print(num_of_partition)
